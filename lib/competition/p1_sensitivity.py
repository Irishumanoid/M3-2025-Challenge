...
house_type_temps_init = house_data
...
for j in range(5):
    new_A = [0, 0, 0, 0]
    for k in range(4):
        new_A[k] = A[k] * (random.random() * 0.1 + 0.95)
    ...
    for i in range(len(A)):
        ...
        for row in range(len(df) - 1):
            percent_diff = (house_data[i][row] - house_type_temps_init[i][row])/house_type_temps_init[i][row]
            sum += percent_diff
        sum = sum / len(df)
        house_sums[i] += sum


house_sums[0] = house_sums[0] / 5
house_sums[1] = house_sums[1] / 5
house_sums[2] = house_sums[2] / 5
house_sums[3] = house_sums[3] / 5
